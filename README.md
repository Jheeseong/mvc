...
### OSIV ON
![image](https://user-images.githubusercontent.com/96407257/149935947-494f9b0f-7d59-4cbe-89cb-f5644ad6ddb1.png)  
- spring.jpa.open-in-view : true 기본값
- OSIV 전략은 트랜잭션 시작처럼 최초 데이터베이스 커넥션 시작 시점부터 API 응답이 끝날 때까지 영속성 컨텍스트와 데이터베이스 커넥션을 유지 -> View Template이나 API컨트롤러에서 지연로딩이 가능한 이유
- 지연 로딩은 영속성 컨텍스트가 살아있어야 가능하고, 영속성 컨텍스트는 기본적으로 데이터베이스 커넥션을 유지
- 문제점
  - 오랫동안 데이터베이스 커넥션 리소스를 사용하기 때문에, 실시간 트래픽이 중요한 애플리케이션에서는 장애 발생, ex) 컨트롤러에서 외부 API를 호출하면 외부 API 대기 시간 만큼 커넥션 리소스를 반환하지 못하고 유지 -> 장애 발생

### OSIV OFF
![image](https://user-images.githubusercontent.com/96407257/149936507-5b0274e8-1349-4640-97fb-059e4d5ab178.png)  
- spring.jpa.open-in-view : false OSIV 종료
- OSIV를 종료 시 트랜잭션을 종료할 때 영속성 컨텍스트를 닫고 데이터베이스 커넥션도 반환 -> 커넥션 리소스 낭비 X
- 문제점
  - OSIV를 종료 시 모든 지연로딩을 트랜잭션 안에서 처리
  - 지금까지 작성한 지연 로딩 코드를 트랜잭션 안으로 넣어야하는 단점
  - view template에서 지연로딩 동작 X
  - 결론적으로 트랜잭션이 끝나기 전에 지연 로딩을 강제로 호출

### 커멘드와 쿼리 분리(CPRS 패턴)
- commend와 Query를 분리하여 **OSIV를 끈 상태**로 복잡성을 관리하기 좋은 패턴
- 비지니스 로직은 특정 엔티티를 등록하거나 수정하는 것으로 성능에 문제 X
- 복잡한 화면을 출력하기 위한 쿼리(조회 등)는 화면에 맞춰 성능을 최적화 하는 것이 중요
- 이 둘의 관심사를 명확하게 분리하는 선택은 유지보수 관점에서 유용
- ex) OrderService
